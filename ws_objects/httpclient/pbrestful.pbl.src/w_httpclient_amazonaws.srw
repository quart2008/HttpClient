...
crypterObject 	lu_md5
coderObject 	lu_code

lu_md5=create crypterObject
lu_code = Create coderObject

ls_new_text = "20171117T051225Z"
ls_result = lu_code.HexEncode(lu_md5.Md5(Blob( ls_new_text, EncodingUTF8! )))
...
long textcolor = 128
long backcolor = 67108864
string text = "1.点GetToken按钮执行,(注意:X-Amz-Date为格林威治时间)"
boolean focusrectangle = false
end type
...
String 	ls_AccessKey, ls_SecretKey
String 	ls_datetime, ls_Date, ls_sign0, ls_sign1, ls_sign
String 	ls_Region, ls_Service, ls_Request
...
sle_Date.text = ls_datetime
ls_Region = "us-east-1"
ls_Service = "execute-api"
ls_Request = "aws4_request"

//获取签名串0
ls_sign0 = ls_Method + "~n" + &
	"/appeon/pets" + "~n" + &
	"~n" + &
	"content-type:application/x-www-form-urlencoded" + "~n" + &
	"host:hk1egzaeia.execute-api.us-east-1.amazonaws.com" + "~n" + & 
	"x-amz-date:" + ls_datetime + "~n" + &
	"~n" + &
	"content-type;host;x-amz-date" + "~n" + &
	"e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855"
	//lco_Code.HexEncode(lco_Crypt.SHA( SHA256!, Blob("",EncodingUTF8!))) 
	// =>e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855 //现在报错
				
//获取签名串1
ls_sign1 = "AWS4-HMAC-SHA256" + "~n"+ &
	ls_datetime + "~n" + &
	ls_date + "/" + ls_region + "/" + ls_service + "/" + ls_request + "~n" + &
	Lower(lco_Code.HexEncode(lco_Crypt.SHA( SHA256!, Blob(ls_sign0,EncodingUTF8!))))
...
lb_sign = lco_Crypt.HMAC( HMACSHA256!, Blob(ls_sign1,EncodingUTF8!), lb_kSigning)
...
If IsNull ( ls_Authorization ) Or Trim ( ls_Authorization ) = "" Then
	...
	Return 
End If
sle_authorization.Text = ls_Authorization
//ls_Data = Trim ( sle_Date.text )
If IsNull ( ls_datetime ) Or Trim ( ls_datetime ) = "" Then
...
lhc_Client.SetRequestHeader ( "X-Amz-Date", ls_datetime )
