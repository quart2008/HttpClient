...
public subroutine wf_scriptexe5 (string as_item, boolean ab_execute)
...
",P046_Uo_HttpClient,P047_Nvo_HttpClient"

//设置测试总体描述
is_testpurpose = "HttpClient test ~r~nruntime时注意检查config.ini文件是否存在"

is_Restful_http_DotNet = profilestring("config.ini","settings","Restful_http_DotNet","") //.Net Restful
If Len(is_Restful_http_DotNet) <= 0 Then
	MessageBox( "Tip","config.ini not found!" )
End If
...
If Appeongetclienttype() = "PB" Then
	is_expfile = "exp.txt"
Else
	is_expfile = appeongetcachedir()+"/plugin/exp.txt"
End If
...
Choose case true
	...
	Case POS ( ls_item, "_fnRequest_" ) > 0
		wf_ScriptExe2 ( ls_item, ab_execute )
	Case POS ( ls_item, "_fnResponse_" ) > 0
		wf_ScriptExe3 ( ls_item, ab_execute )
	Case POS ( ls_item, "_Properties_" ) > 0 
		wf_ScriptExe4 ( ls_item, ab_execute )
	Case POS ( ls_item, "o_HttpClient" ) > 0
		wf_ScriptExe5 ( ls_item, ab_execute )
end choose
...
Choose Case as_item
	...
	Case 'P012_fnRequest_SetRequestHeader'
		If ab_execute = True Then
			...
			wf_output( 'SetRequestHeader(space(5000), "*/*") ' + " Return:" + String (li_rtn ), False)
	...
	Case 'P026_fnRequest_PostData'
		If ab_execute = True Then
			...
			Else
				flen = FileLength(appeongetcachedir()+"/plugin/pic2.bmp")
				ll_Num = FileOpen(appeongetcachedir()+"/plugin/pic2.bmp", StreamMode!, Read!, LockRead!)				
			End If
			If ll_Num = -1 Then
			...
			Else
				ll_Num = FileOpen(appeongetcachedir()+"/plugin/md5.txt", StreamMode!, Read!, LockRead!)				
			End If
...
String ls_item, ls_value,ls_Respose, ls_Body, ls_txt
Long			ll_rtn, ll_Code
...
Choose Case as_item	
	//P046_Uo_HttpClient
	Case 'P046_Uo_HttpClient'
		If ab_execute = True Then
			uo_HttpClient luo_HttpClient
			luo_HttpClient = Create uo_HttpClient
						
			ll_rtn = luo_HttpClient.SendRequest( "GET", is_Restful_http_DotNet + "/api/Token/GetToken?staffId=10000" )
			If ll_rtn = 1 Then
				ls_Respose = luo_HttpClient.Getresponseheaders( )
				wf_OutPut("uo_HttpClient SendRequest GetResponseHeaders() = "+String ( ls_Respose), False)
				ll_rtn = luo_HttpClient.GetResponsebody( ls_Body)
				wf_OutPut("uo_HttpClient SendRequest GetResponsebody() = "+String ( ls_Body), False)
				ll_Code = luo_HttpClient.GetResponseStatusCode()
				wf_OutPut("uo_HttpClient SendRequest GetResponseStatusCode() = "+String ( ll_Code), False)
				ls_txt = luo_HttpClient.GetResponseStatusText()
				wf_OutPut("uo_HttpClient SendRequest GetResponseStatusText() = "+String ( ls_txt), False)
			Else
				wf_OutPut("uo_HttpClient SendRequest Failed" + " Return:" + String ( ll_rtn ), False)
			End If
			
			If IsValid ( luo_HttpClient ) Then DesTroy ( luo_HttpClient )
		Else
			wf_scriptview("uo_HttpClient luo_HttpClient~r~n" +&
					"luo_HttpClient = Create uo_HttpClient~r~n" +&
					"			~r~n" +&
					"ll_rtn = luo_HttpClient.SendRequest( ~"GET~",~"" +  is_Restful_http_DotNet + "/api/Token/GetToken?staffId=10000~" )~r~n" +&
					"If ll_rtn = 1 Then~r~n" +&
					"	ls_Respose = luo_HttpClient.Getresponseheaders( )~r~n" +&
					"	wf_OutPut(~"uo_HttpClient SendRequest GetResponseHeaders() = ~"+String ( ls_Respose), False)~r~n" +&
					"	ll_rtn = luo_HttpClient.GetResponsebody( ls_Body)~r~n" +&
					"	wf_OutPut(~"uo_HttpClient SendRequest GetResponsebody() = ~"+String ( ls_Body), False)~r~n" +&
					"	ll_Code = luo_HttpClient.GetResponseStatusCode()~r~n" +&
					"	wf_OutPut(~"uo_HttpClient SendRequest GetResponseStatusCode() = ~"+String ( ll_Code), False)~r~n" +&
					"	ls_txt = luo_HttpClient.GetResponseStatusText()~r~n" +&
					"	wf_OutPut(~"uo_HttpClient SendRequest GetResponseStatusText() = ~"+String ( ls_txt), False)~r~n" +&
					"Else~r~n" +&
					"	wf_OutPut(~"uo_HttpClient SendRequest Failed~" + ~" Return:~" + String ( ll_rtn ), False)~r~n" +&
					"End If~r~n" +&
					"~r~n" +&
					"If IsValid ( luo_HttpClient ) Then DesTroy ( luo_HttpClient )~r~n")
		End If
		//P047_Nvo_HttpClient
	Case 'P047_Nvo_HttpClient'
		If ab_execute = True Then
			//HttpClient_1 //Non-Visual Object
			ll_rtn = HttpClient_1.SendRequest( "GET", is_Restful_https_DotNet + "/api/Token/GetToken?staffId=10000" )
			If ll_rtn = 1 Then
				ls_Respose = HttpClient_1.Getresponseheaders( )
				wf_OutPut("HttpClient_1 SendRequest GetResponseHeaders() = "+String ( ls_Respose), False)
				ll_rtn = HttpClient_1.GetResponsebody( ls_Body)
				wf_OutPut("HttpClient_1 SendRequest GetResponsebody() = "+String ( ls_Body), False)
				ll_Code = HttpClient_1.GetResponseStatusCode()
				wf_OutPut("HttpClient_1 SendRequest GetResponseStatusCode() = "+String ( ll_Code), False)
				ls_txt = HttpClient_1.GetResponseStatusText()
				wf_OutPut("HttpClient_1 SendRequest GetResponseStatusText() = "+String ( ls_txt), False)
			Else
				wf_OutPut("HttpClient_1 SendRequest Failed" + " Return:" + String ( ll_rtn ), False)
			End If
		Else
			wf_scriptview("//HttpClient_1 //Non-Visual Object~r~n" +&
					"ll_rtn = HttpClient_1.SendRequest( ~"GET~",~"" +  is_Restful_https_DotNet + "/api/Token/GetToken?staffId=10000~" )~r~n" +&
					"If ll_rtn = 1 Then~r~n" +&
					"	ls_Respose = HttpClient_1.Getresponseheaders( )~r~n" +&
					"	wf_OutPut(~"HttpClient_1 SendRequest GetResponseHeaders() = ~"+String ( ls_Respose), False)~r~n" +&
					"	ll_rtn = HttpClient_1.GetResponsebody( ls_Body)~r~n" +&
					"	wf_OutPut(~"HttpClient_1 SendRequest GetResponsebody() = ~"+String ( ls_Body), False)~r~n" +&
					"	ll_Code = HttpClient_1.GetResponseStatusCode()~r~n" +&
					"	wf_OutPut(~"HttpClient_1 SendRequest GetResponseStatusCode() = ~"+String ( ll_Code), False)~r~n" +&
					"	ls_txt = HttpClient_1.GetResponseStatusText()~r~n" +&
					"	wf_OutPut(~"HttpClient_1 SendRequest GetResponseStatusText() = ~"+String ( ls_txt), False)~r~n" +&
					"Else~r~n" +&
					"	wf_OutPut(~"HttpClient_1 SendRequest Failed~" + ~" Return:~" + String ( ll_rtn ), False)~r~n" +&
					"End If~r~n")
		End If
	Case Else	
	 	MessageBox("Error","This Point Not Coding",Exclamation!)
...
Choose Case as_item	
	...
	Case 'P039_Properties_SecureProtocol'
		If ab_execute = True Then
			...
			ll_rtn = lhc_Client.SendRequest("GET", "https://192.0.0.47/employee/getemployees")
			If ll_rtn = 1 Then
			...
			Try
				...
				End If
			Catch (throwable ex1)
				wf_output( 'Http 6 SendRequest Failed for secureprotocol = 6 Return: ' + ex1.GetMessage(), False)
			End Try
			//-1
			Try
				...
				End If
			Catch (throwable ex2)
				wf_output( 'Http -1 SendRequest Failed for secureprotocol = -1 Return: ' + ex2.GetMessage(), False)
			End Try
		Else
			wf_scriptview("//SecureProtocol~r~n" +&
					"Integer li_ProTocol~r~n" +&
					"Long ll_I,ll_Code~r~n" +&
					...
					"ll_rtn = lhc_Client.SendRequest(~"GET~", ~"https://192.0.0.47/employee/getemployees~")~r~n" +&
					"If ll_rtn = 1 Then~r~n" +&
					...
					"	wf_output( ~'Http 6 SendRequest Failed for secureprotocol = 6 Return: ~' + ex1.GetMessage(), False)~r~n" +&
					"End Try~r~n" +&
					"//-1~r~n" +&
					"Try~r~n" +&
					...
					"	wf_output( ~'Http -1 SendRequest Failed for secureprotocol = -1 Return: ~' + ex2.GetMessage(), False)~r~n" +&
					"End Try~r~n")
	...
	Case 'P045_Properties_Timeout'
		If ab_execute = True Then
			...
			//无效url
			wf_OutPut("测试无效url的timeout情况:",False)
			lhc_Client.Timeout = 1
			ll_cpu = cpu()
			ll_rtn = lhc_Client.sendrequest("GET", "http://192.254.254.254" )
			wf_OutPut('Set Timeout = 1' + " 无效Url Return:" + String ( ll_rtn ) + " TimeOut:" + String ( cpu() - ll_cpu ), False)
			
			lhc_Client.Timeout = 2
			ll_cpu = cpu()
			ll_rtn = lhc_Client.sendrequest("GET", "http://192.254.254.254" )
			wf_OutPut('Set Timeout = 2' + " 无效Url Return:" + String ( ll_rtn ) + " TimeOut:" + String ( cpu() - ll_cpu ), False)
			
			lhc_Client.Timeout = 5
			ll_cpu = cpu()
			ll_rtn = lhc_Client.sendrequest("GET", "http://192.254.254.254" )
			wf_OutPut('Set Timeout = 5' + " 无效Url Return:" + String ( ll_rtn ) + " TimeOut:" + String ( cpu() - ll_cpu ), False)
			
			lhc_Client.Timeout = 10
			ll_cpu = cpu()
			ll_rtn = lhc_Client.sendrequest("GET", "http://192.254.254.254" )
			wf_OutPut('Set Timeout = 10' + " 无效Url Return:" + String ( ll_rtn ) + " TimeOut:" + String ( cpu() - ll_cpu ), False)
		...
		Else
			wf_scriptview("//Timeout~r~n" +&
					...
					"~r~n" +&
					"//无效url~r~n" +&
					"wf_OutPut(~"测试无效url的timeout情况:~",False)~r~n" +&
					"lhc_Client.Timeout = 1~r~n" +&
					"ll_cpu = cpu()~r~n" +&
					"ll_rtn = lhc_Client.sendrequest(~"GET~", ~"http://192.254.254.254~" )~r~n" +&
					"wf_OutPut(~'Set Timeout = 1~' + ~" 无效Url Return:~" + String ( ll_rtn ) + ~" TimeOut:~" + String ( cpu() - ll_cpu ), False)~r~n" +&
					"~r~n" +&
					"lhc_Client.Timeout = 2~r~n" +&
					"ll_cpu = cpu()~r~n" +&
					"ll_rtn = lhc_Client.sendrequest(~"GET~", ~"http://192.254.254.254~" )~r~n" +&
					"wf_OutPut(~'Set Timeout = 2~' + ~" 无效Url Return:~" + String ( ll_rtn ) + ~" TimeOut:~" + String ( cpu() - ll_cpu ), False)~r~n" +&
					"~r~n" +&
					"lhc_Client.Timeout = 5~r~n" +&
					"ll_cpu = cpu()~r~n" +&
					"ll_rtn = lhc_Client.sendrequest(~"GET~", ~"http://192.254.254.254~" )~r~n" +&
					"wf_OutPut(~'Set Timeout = 5~' + ~" 无效Url Return:~" + String ( ll_rtn ) + ~" TimeOut:~" + String ( cpu() - ll_cpu ), False)~r~n" +&
					"~r~n" +&
					"lhc_Client.Timeout = 10~r~n" +&
					"ll_cpu = cpu()~r~n" +&
					"ll_rtn = lhc_Client.sendrequest(~"GET~", ~"http://192.254.254.254~" )~r~n" +&
					"wf_OutPut(~'Set Timeout = 10~' + ~" 无效Url Return:~" + String ( ll_rtn ) + ~" TimeOut:~" + String ( cpu() - ll_cpu ), False)~r~n")
		End If
